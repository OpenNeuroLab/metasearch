...
function scatterplot() {
  ...
  function scatter(selection) {
    ...
    svg = d3.select(selection).attr("width", w + m[1] + m[3]).attr("height", w + m[0] + m[2]).append("g").attr("transform", "translate(" + m[3] + "," + m[0] + ")");
    ...
    dimensions = _(columns).difference(excluded);

    // extents for each dimension
    extents = _(dimensions).map(function (col) {
  ...
  scatter.show = function (shown) {
    update = svg.selectAll('circle').style("display", function (d) {
  ...
  scatter.update = function () {
    ...
    var update = svg.selectAll('circle').data(data, key ? key : undefined);

    update.enter().append('circle').attr("fill", function (d) {
      return color[d.group];
    }).attr("cx", function (d) {
      return x(d[dimensions[xcol]]);
    }).attr("cy", function (d) {
      return y(d[dimensions[ycol]]);
...
//# sourceMappingURL=scatterplot-compiled.js.map