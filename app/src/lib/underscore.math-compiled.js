...
(function () {
  ...
  math.pow = function (x, n) {
    if (_.isNumber(x)) return Math.pow(x, n);
    if (_.isArray(x)) return _.map(x, function (i) {
      return _.pow(i, n);
    });
  };
  ...
  math.variance = function (arr) {
    var mean = _(arr).mean();
    var variance = function variance(x) {
      return _(x - mean).pow(2);
    };
...
//# sourceMappingURL=underscore.math-compiled.js.map